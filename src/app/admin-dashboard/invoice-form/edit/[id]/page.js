...
export default function NewBookingForm({ invoiceId = null }) {
  ...
  // Fetch existing invoice data if invoiceId is provided
  useEffect(() => {
    if (invoiceId) {
      const fetchInvoiceData = async () => {
        try {
          const response = await fetch(`/api/admin/invoice-management?id=${invoiceId}`);
          ...
          setInvoiceData({
            ...
            vehicles: data.vehicles.map((v) => ({
              ...v,
              year: String(v.year),
              documentReceiveDate: v.documentReceiveDate ? v.documentReceiveDate.split("T")[0] : null,
              ownershipDate: v.ownershipDate ? v.ownershipDate.split("T")[0] : null,
              vehicleImages: v.vehicleImages.map((img) => img.imagePath),
              vehicleImagePreviews: v.vehicleImages.map((img) => img.imagePath),
            })),
          });
          setInvoiceImagePreview(data.imagePath || null);
        } catch (err) {
      ...
      setLoading(false); // No invoiceId, proceed without fetching
    }
  }, [invoiceId]);
  ...
  // Fetch initial data once on mount (only if not fetching invoice data)
  useEffect(() => {
    if (!invoiceId) {
  ...
  const handleInputChange = (field, value) => {
    setInvoiceData((prev) => {
      ...
      if (field === "amountYen") {
        ...
        const file = value[0];
        setInvoiceImagePreview(file ? URL.createObjectURL(file) : value); // Handle both file and string (URL)
      }
      return updatedData;
    });
  };
  ...
  return (
    <Box sx={{ p: 2, bgcolor: "#FFFFFF" }}>
      <Typography variant="h4" gutterBottom>
        {invoiceId ? "Edit Invoice" : "Edit Vehicle Booking"}
      </Typography>
          ...
          <Paper key={index} elevation={1} sx={{ p: 2, mb: 2, bgcolor: "#f5f5f5" }}>
            <Box display="grid" gridTemplateColumns="repeat(6, 1fr)" gap={2}>
              ...
              <TextField
                ...
                variant="outlined"
                value={vehicle.documentReceiveDate || ""}
      ...
      >
        {submitting ? "Submitting..." : invoiceId ? "Update Invoice" : "Submit Invoice"}
      </Button>
    </Box>
  );
}